  ...
  <div class="pending-exceptions-tab bg-white border rounded p-4 mb-4">
    ...
    <div v-else-if="exceptions.length === 0" class="text-center py-5 text-muted">
      ...
      <div class="d-flex flex-column gap-3">
        <div v-for="req in exceptions" :key="req.id" class="card shadow-sm border-0 glass-panel hover-lift">
          <div class="card-body d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
            
            <div>
              <div class="d-flex align-items-center gap-2 mb-2">
                <span class="badge rounded-pill shadow-sm" :class="req.type === 'TimeOff' ? 'bg-danger bg-opacity-75' : 'bg-info text-dark bg-opacity-75'">
                  <i :class="req.type === 'TimeOff' ? 'bi bi-calendar-minus-fill' : 'bi bi-arrow-left-right'"></i>
                  {{ req.type === 'TimeOff' ? ' Xin Nghỉ Phép' : ' Xin Đổi Ca' }}
                </span>
                <span class="fw-bold text-dark fs-6">
                  <i class="bi bi-person-circle text-muted me-1"></i> Bs. {{ getDoctorName(req.doctorId) }}
                </span>
              </div>
              
              <div class="text-muted small">
                <div class="mb-1">
                  <i class="bi bi-calendar-range text-warning me-1"></i> 
                  Từ <strong>{{ formatDate(req.startDate) }}</strong> đến <strong>{{ formatDate(req.endDate) }}</strong>
                </div>
                <div v-if="req.type === 'ShiftSwap'" class="mb-1">
                  <i class="bi bi-person-up text-info me-1"></i> 
                  <strong>Trực thay:</strong> Bs. {{ getDoctorName(req.substituteDoctorId) }}
                </div>
                <div v-if="req.reason">
...
.hover-lift:hover {
  ...
  box-shadow: 0 8px 15px rgba(245, 158, 11, 0.1) !important;
}
</style>
